...
class TestUserModel:
    ...
    def test_usermodel_create_method_only_sets_users_as_is_active(self, user_model, create_mth):
    ...
    @pytest.mark.django_db
    def test_usermodel_create_user_method_only_sets_users_as_is_active(self, user_model, create_user_mth):
        # because of the create_mth fixture
        assert user_model.objects.count() == 1
    ...
    @pytest.mark.django_db
    def test_usermodel_create_superuser_method_gives_users_all_privilege(self, user_model, create_superuser_mth):
        # because of the create_mth fixture
        assert user_model.objects.count() == 1
    ...
    def test_usermodel_str_method_returns_username_and_app_name_if_present(
        self, user_model, create_mth, create_superuser_mth
    ):
        ...
        assert str(create_superuser_mth) == F"{username} ()"

    @pytest.mark.django_db
    def test_usermodel_app_name_field_obeys_max_length_validator(self, user_model):
        ...
        test_data = dict(
            ...
            aid=self.fake.uuid4(),
            app_name="this app name is more than 20 chars",  # should not be more than 20 char
            password=self.fake.password(length=10)
        )

        with pytest.raises(db_exception.DataError) as exc_info:
            user = user_model.objects.create(**test_data)
    ...
    @pytest.mark.django_db
    def test_usermodel_username_field_obeys_max_length_validator(self, user_model):
        assert user_model.objects.count() == 0

        test_data = dict(
            username="ThisUserNameFieldToLong",
            email=self.fake.safe_email(),
            cid=self.fake.ean(length=13),
            aid=self.fake.uuid4(),
    ...
    @pytest.mark.django_db
    def test_usermodel_username_and_email_fields_are_converted_to_lowercase(self, user_model, user_data_2, create_mth):
        # because of the create_mth fixture
        assert user_model.objects.count() == 1
    ...
    def test_usermodel_username_and_email_fields_should_be_unique(self, user_model):
        assert user_model.objects.count() == 0

        # test data
        test_data_1 = dict(username="testUserOne", email="TestUserOne@email.com", password="p@ssw0rd")
        test_data_1_2 = dict(username="testUserOne", email="TestUserTwo@email.com", password="p@ssw0rd")
        test_data_2_1 = dict(username="testUserTwo", email="TestUserOne@email.com", password="p@ssw0rd")

        # create a user with the test_data_1 object
        user_model.objects.create_user(**test_data_1)
        ...
        with pytest.raises(db_exception.IntegrityError) as exc_info:
            user_model.objects.create_user(**test_data_1_2)
        ...
        with pytest.raises(db_exception.IntegrityError) as exc_info:
            user_model.objects.create_user(**test_data_2_1)
